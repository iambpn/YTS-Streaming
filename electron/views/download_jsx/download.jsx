...
function Download() {
  ...
  React.useEffect(() => {
    window.api.receive('download:info', (data) => {
      ...
      data.uploadSpeed = data.uploadSpeed.slice(
        0,
        data.uploadSpeed.indexOf('.') + 3
      ); // Slice

      data.downloadSpeed = (data.downloadSpeed / (1000 * 1000)).toString();
      data.downloadSpeed = data.downloadSpeed.slice(
        0,
        data.downloadSpeed.indexOf('.') + 3
      );
      ...
      data.totalDownloaded = data.totalDownloaded.slice(
        0,
        data.totalDownloaded.indexOf('.') + 4
      );

      data.downloadSize = (data.downloadSize / 1000 ** 3).toString();
      data.downloadSize = data.downloadSize.slice(
        0,
        data.downloadSize.indexOf('.') + 4
      );
      ...
      <div className="w-100">
        <div className="row g-0 justify-content-between w-100">
          <div className="col-8 g-0">
            <h6 className="text-truncate w-100 d-inline-block mb-0">
              Downloading:
          ...
          <div className="col-4 g-0 text-end" style={{ fontSize: '16px' }}>
            <span className="pr-1">
              <span id="up_speed">{apiData.uploadSpeed}</span> &uarr;M
            </span>
            <span>/</span>
            <span className="pr-2">
              <span id="down_speed">{apiData.downloadSpeed}</span> &darr;M
            </span>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-between w-100">
          ...
          <div>
            {isPaused ? (
              <button
                className="btn btn-outline-primary m-1 mt-2"
                id="resume"
              ...
              <button
                className="btn btn-outline-warning m-1 mt-2"
                id="pause"
            ...
            <button
              className="btn btn-outline-danger m-1 mt-2"
              id="cancel"
